(function () {
    ...
    const TRANSITIONS = {
        curtain: {
            label: 'Curtain Slide',
            schema: {
                direction: { phase: 'shared', type: 'select', label: 'Direction', options: ['up', 'down', 'left', 'right'], default: 'up' },
                color:     { phase: 'shared', type: 'color',  label: 'Panel color', default: '#0a0a0a' },
                ...TIMING(0.85, 1.00, 'power3.inOut')
            },
            build(o, c) {
                o.innerHTML = '<div class="pt-panel"></div>';
                o.querySelector('.pt-panel').style.background = c.color;
            },
            _prop(c) { return (c.direction === 'left' || c.direction === 'right') ? 'xPercent' : 'yPercent'; },
            _exitFrom(c) { return { up: 100, down: -100, left: 100, right: -100 }[c.direction]; },
            _enterTo(c)  { return { up: -100, down: 100, left: -100, right: 100 }[c.direction]; },
            arrive(o) {
        ...
        split: {
            label: 'Split Reveal',
            schema: {
                axis:  { phase: 'shared', type: 'select', label: 'Axis', options: ['vertical', 'horizontal'], default: 'vertical' },
                color: { phase: 'shared', type: 'color',  label: 'Panel color', default: '#0a0a0a' },
                ...TIMING(0.9, 1.0, 'power3.inOut')
            },
            build(o, c) {
                o.setAttribute('data-axis', c.axis);
                o.innerHTML = '<div class="pt-split-a"></div><div class="pt-split-b"></div>';
                o.querySelectorAll('.pt-split-a, .pt-split-b').forEach(el => el.style.background = c.color);
            },
            _prop(c) { return c.axis === 'horizontal' ? 'xPercent' : 'yPercent'; },
            arrive(o) {
                gsap.set(o.querySelectorAll('.pt-split-a, .pt-split-b'), { xPercent: 0, yPercent: 0 });
            },
            enter(o, c) {
                const p = this._prop(c);
                const tl = gsap.timeline();
                tl.to(o.querySelector('.pt-split-a'), { [p]: -100, duration: c.enterDuration, ease: c.enterEase }, 0)
                  .to(o.querySelector('.pt-split-b'), { [p]:  100, duration: c.enterDuration, ease: c.enterEase }, 0);
                return tl;
            },
            exit(o, c) {
                const p = this._prop(c);
                const a = { xPercent: 0, yPercent: 0 }; a[p] = -100;
                const b = { xPercent: 0, yPercent: 0 }; b[p] =  100;
                gsap.set(o.querySelector('.pt-split-a'), a);
                gsap.set(o.querySelector('.pt-split-b'), b);
                const tl = gsap.timeline();
                tl.to(o.querySelector('.pt-split-a'), { [p]: 0, duration: c.exitDuration, ease: c.exitEase }, 0)
                  .to(o.querySelector('.pt-split-b'), { [p]: 0, duration: c.exitDuration, ease: c.exitEase }, 0);
                return tl;
            }
        },
        ...
        stagger: {
            label: 'Panel Stagger',
            schema: {
                count:            { phase: 'shared', type: 'range',  label: 'Panel count', min: 2, max: 12, step: 1, default: 6 },
            ...
            build(o, c) {
                ...
                o.innerHTML = Array.from({ length: c.count }).map((_, i) => {
                    const bg = i % 2 ? alt : c.color;
                    return `<div class="pt-bar" style="background:${bg};box-shadow:0 0 0 1px ${bg}"></div>`;
                }).join('');
            },
            _signs(c) { return c.direction === 'down' ? { from: -100, to: 100 } : { from: 100, to: -100 }; },
            arrive(o) {
            ...
            exit(o, c) {
                gsap.set(o.querySelectorAll('.pt-bar'), { yPercent: this._signs(c).from });
                return gsap.to(o.querySelectorAll('.pt-bar'), {
                    yPercent: 0,
        ...
        fade: {
            label: 'Fade',
            schema: {
                color: { phase: 'shared', type: 'color', label: 'Overlay color', default: '#0a0a0a' },
            ...
            enter(o, c) {
                return gsap.to(o.querySelector('.pt-panel'), {
                    autoAlpha: 0, duration: c.enterDuration, ease: c.enterEase
                });
            },
            exit(o, c) {
                gsap.set(o.querySelector('.pt-panel'), { autoAlpha: 0 });
                return gsap.to(o.querySelector('.pt-panel'), {
                    autoAlpha: 1, duration: c.exitDuration, ease: c.exitEase
                });
            }
        },
        ...
        diagonal: {
            ...
            schema: {
                angle: { phase: 'shared', type: 'range', label: 'Angle', min: -45, max: 45, step: 1, unit: '°', default: -18 },
                color: { phase: 'shared', type: 'color', label: 'Panel color', default: '#0a0a0a' },
                ...TIMING(0.95, 1.10, 'power3.inOut')
            },
            build(o, c) {
                o.innerHTML = '<div class="pt-panel"></div>';
                o.querySelector('.pt-panel').style.background = c.color;
            ...
            arrive(o, c) {
                gsap.set(o.querySelector('.pt-panel'), { xPercent: 0, yPercent: 0, rotation: c.angle });
            },
            enter(o, c) {
                return gsap.to(o.querySelector('.pt-panel'), {
                    xPercent: -80, yPercent: -80, rotation: c.angle,
                    duration: c.enterDuration, ease: c.enterEase
                });
            },
            exit(o, c) {
                gsap.set(o.querySelector('.pt-panel'), { xPercent: 80, yPercent: 80, rotation: c.angle });
                return gsap.to(o.querySelector('.pt-panel'), {
                    xPercent: 0, yPercent: 0, rotation: c.angle,
                    duration: c.exitDuration, ease: c.exitEase
                });
            }
        },
        ...
        circle: {
            label: 'Circle Reveal',
            schema: {
                origin: { phase: 'shared', type: 'select', label: 'Origin', options: ['center','top-left','top-right','bottom-left','bottom-right','top','bottom','left','right'], default: 'center' },
                color:  { phase: 'shared', type: 'color',  label: 'Circle color', default: '#0a0a0a' },
                ...TIMING(0.9, 1.0, 'power3.inOut')
            },
            build(o, c) {
            ...
            enter(o, c) {
                return gsap.to(o.querySelector('.pt-circle'), {
                    scale: 0, duration: c.enterDuration, ease: c.enterEase
                });
            },
            exit(o, c) {
                gsap.set(o.querySelector('.pt-circle'), { scale: 0 });
                return gsap.to(o.querySelector('.pt-circle'), {
                    scale: 35, duration: c.exitDuration, ease: c.exitEase
                });
            }
        },
        ...
        scale: {
            ...
            schema: {
                origin:     { phase: 'shared', type: 'select', label: 'Origin', options: ['center','top-left','top-right','bottom-left','bottom-right','top','bottom','left','right'], default: 'center' },
                startScale: { phase: 'shared', type: 'range',  label: 'Start scale', min: 0, max: 0.6, step: 0.02, default: 0 },
                color:      { phase: 'shared', type: 'color',  label: 'Panel color', default: '#0a0a0a' },
                ...TIMING(0.85, 1.00, 'power3.inOut')
            },
            build(o, c) {
                ...
                const origins = {
                    'center': '50% 50%', 'top-left': '0% 0%', 'top-right': '100% 0%',
        ...
        flip: {
            label: 'Flip 3D',
            schema: {
                axis:        { phase: 'shared', type: 'select', label: 'Axis', options: ['y', 'x'], default: 'y' },
                perspective: { phase: 'shared', type: 'range',  label: 'Perspective', min: 400, max: 2400, step: 50, unit: 'px', default: 1200 },
                color:       { phase: 'shared', type: 'color',  label: 'Panel color', default: '#0a0a0a' },
                ...TIMING(0.85, 1.00, 'power3.inOut')
            },
            build(o, c) {
            ...
            _prop(c) { return c.axis === 'x' ? 'rotationX' : 'rotationY'; },
            arrive(o) {
        ...
        clip: {
            label: 'Clip Wipe',
            schema: {
                direction: { phase: 'shared', type: 'select', label: 'Direction', options: ['from-top','from-bottom','from-left','from-right','inset-horizontal','inset-vertical','inset-center'], default: 'from-top' },
                color:     { phase: 'shared', type: 'color',  label: 'Panel color', default: '#0a0a0a' },
                ...TIMING(0.90, 1.00, 'power3.inOut')
            },
            build(o, c) {
                o.innerHTML = '<div class="pt-panel"></div>';
                o.querySelector('.pt-panel').style.background = c.color;
            ...
            _clips(c) {
                ...
                const map = {
                    'from-top':         { hide: 'inset(0% 0% 100% 0%)',   out: 'inset(100% 0% 0% 0%)' },
                    'from-bottom':      { hide: 'inset(100% 0% 0% 0%)',   out: 'inset(0% 0% 100% 0%)' },
                    'from-left':        { hide: 'inset(0% 100% 0% 0%)',   out: 'inset(0% 0% 0% 100%)' },
                    'from-right':       { hide: 'inset(0% 0% 0% 100%)',   out: 'inset(0% 100% 0% 0%)' },
                    'inset-horizontal': { hide: 'inset(0% 50% 0% 50%)',   out: 'inset(0% 50% 0% 50%)' },
                    'inset-vertical':   { hide: 'inset(50% 0% 50% 0%)',   out: 'inset(50% 0% 50% 0%)' },
                    'inset-center':     { hide: 'inset(50% 50% 50% 50%)', out: 'inset(50% 50% 50% 50%)' }
                };
                const d = map[c.direction] || map['from-top'];
                return { hide: d.hide, show: SHOW, out: d.out };
            },
            arrive(o, c) {
                gsap.set(o.querySelector('.pt-panel'), { clipPath: this._clips(c).show });
            },
            enter(o, c) {
                return gsap.to(o.querySelector('.pt-panel'), {
            ...
            exit(o, c) {
                gsap.set(o.querySelector('.pt-panel'), { clipPath: this._clips(c).hide });
                return gsap.to(o.querySelector('.pt-panel'), {
        ...
        crossfade: {
            label: 'Crossfade',
            schema: {
                opacity: { phase: 'shared', type: 'range', label: 'Overlay opacity', min: 0.3, max: 1, step: 0.05, default: 1.0 },
                blur:    { phase: 'shared', type: 'range', label: 'Backdrop blur', min: 0, max: 24, step: 1, unit: 'px', default: 6 },
                color:   { phase: 'shared', type: 'color', label: 'Overlay color', default: '#0a0a0a' },
        ...
        tileGrid: {
            ...
            build(o, c) {
                ...
                let html = '';
                for (let i = 0; i < rows * cols; i++) html += '<div class="pt-tile"></div>';
                o.innerHTML = html;
            ...
            enter(o, c) {
                ...
                return gsap.to(o.querySelectorAll('.pt-tile'), {
                    ...
                    duration: c.enterDuration,
                    ease: c.enterEase,
                    stagger: { grid: [rows, cols], from: c.enterStaggerFrom, each: c.enterStaggerEach }
                });
            },
            exit(o, c) {
                const rows = parseInt(o.getAttribute('data-rows'), 10);
                ...
                return gsap.to(o.querySelectorAll('.pt-tile'), {
                    ...
                    duration: c.exitDuration,
                    ease: c.exitEase,
                    stagger: { grid: [rows, cols], from: c.exitStaggerFrom, each: c.exitStaggerEach }
                });
            }
        },
        ...
        shutter: {
            ...
            _prop(c) { return c.axis === 'vertical' ? 'scaleX' : 'scaleY'; },
            arrive(o) {
        ...
        iris: {
            label: 'Iris',
            schema: {
                shape: { phase: 'shared', type: 'select', label: 'Shape', options: ['diamond', 'hexagon', 'square'], default: 'diamond' },
                color: { phase: 'shared', type: 'color',  label: 'Panel color', default: '#0a0a0a' },
                ...TIMING(0.9, 1.0, 'power3.inOut')
            },
            build(o, c) {
        ...
        push: {
            label: 'Push Slide',
            schema: {
                direction:   { phase: 'shared', type: 'select', label: 'Direction', options: ['left', 'right', 'up', 'down'], default: 'left' },
                color:       { phase: 'shared', type: 'color',  label: 'Panel color', default: '#0a0a0a' },
                accentColor: { phase: 'shared', type: 'color',  label: 'Lead color', default: '#c6f432' },
                accentWidth: { phase: 'shared', type: 'range',  label: 'Lead thickness', min: 0, max: 120, step: 2, unit: 'px', default: 14 },
                ...TIMING(0.75, 0.85, 'power3.inOut')
            },
            build(o, c) {
                o.setAttribute('data-direction', c.direction);
                o.innerHTML = '<div class="pt-push-wrap"><div class="pt-push-accent"></div><div class="pt-push-panel"></div></div>';
                ...
                if (isHorizontal) {
                    wrap.style.flexDirection = c.direction === 'left' ? 'row' : 'row-reverse';
                    accent.style.width  = c.accentWidth + 'px';
                    accent.style.height = '100%';
                } else {
                    wrap.style.flexDirection = c.direction === 'up' ? 'column' : 'column-reverse';
            ...
            _prop(c) { return (c.direction === 'up' || c.direction === 'down') ? 'yPercent' : 'xPercent'; },
            _exitFrom(c) { return { left: 100, right: -100, up: 100, down: -100 }[c.direction]; },
            _enterTo(c)  { return { left: -100, right: 100, up: -100, down: 100 }[c.direction]; },
            arrive(o) {
                gsap.set(o.querySelector('.pt-push-wrap'), { xPercent: 0, yPercent: 0 });
            },
            enter(o, c) {
        ...
        custom: {
            ...
            enter(o, c) {
                return runCustomBlock(o.querySelector('.pt-panel'), (c && c.enter) || {});
            },
            exit(o, c) {
                return runCustomBlock(o.querySelector('.pt-panel'), (c && c.exit) || {});
            }
        }
    };
    ...
    function runCustomBlock(target, block) {
        ...
        const gsapFn = ({ from: 'from', to: 'to', fromto: 'fromTo', set: 'set' })[rawMethod] || 'to';
    ...
    const FLAG_TRANSIT = 'pt:transitioning';
    const FLAG_NAME = 'pt:name';
    const FLAG_CONFIG = 'pt:config';

    function getSelected() {
        const s = LS.get(KEY_SELECTED);
        return TRANSITIONS[s] ? s : DEFAULT_NAME;
    }
    function setSelected(n) { LS.set(KEY_SELECTED, n); }
    ...
    function getDefaults(name) {
        const d = {};
        Object.entries(TRANSITIONS[name].schema).forEach(([k, v]) => d[k] = v.default);
        return d;
    }
    ...
    function getConfig(name) {
        ...
        if (name === 'custom') {
            const read = (k) => {
                try { return JSON.parse(LS.get(k) || 'null') || {}; } catch (e) { return {}; }
            };
            return {
    ...
    function setConfig(name, cfg) {
        if (name === 'custom') {
            if (cfg.shared) LS.set(KEY_CFG_SHARED('custom'), JSON.stringify(cfg.shared));
    ...
    function bodyOutVars(effect, strength) {
        ...
        switch (effect) {
            case 'scale':     return { scale:     1 - s * 0.12 };
            case 'fade':      return { opacity:   1 - s * 0.7 };
            case 'blur':      return { filter:    `blur(${s * 12}px)` };
            case 'slideUp':   return { yPercent: -s * 10 };
            case 'slideDown': return { yPercent:  s * 10 };
            default:          return null;
        }
    }
    ...
    function bodyInVars(effect) {
        switch (effect) {
            ...
            case 'blur':      return { filter: 'blur(0px)' };
            case 'slideUp':
            case 'slideDown': return { yPercent: 0 };
            default:          return null;
        }
    }
    ...
    function playBodyOut(global) {
        ...
        return gsap.to(targets, Object.assign({ duration: 0.55, ease: 'power2.inOut' }, vars));
    }
    ...
    const isPreserved = (el) => el && el.classList && PT_PRESERVE.some(c => el.classList.contains(c));
    ...
    async function navigate(href, push) {
        ...
        if (fetchErr) {
            // Fallback path: fetch isn't available (e.g. file:// protocol).
            console.warn('[pt] fetch unavailable — falling back to reload-based nav. Serve the site via a local HTTP server (Live Server, `npx serve`, `python -m http.server`) for flash-free SPA navigation.', fetchErr);
            SS.set(FLAG_TRANSIT, '1');
            SS.set(FLAG_NAME, name);
            try { SS.set(FLAG_CONFIG, JSON.stringify(cfg)); } catch (e) {}
    ...
    const PROP_CATEGORIES = {
        ...
        'Transform': ['x', 'y', 'z', 'xPercent', 'yPercent', 'rotation', 'rotationX', 'rotationY'],
    ...
    const PROP_SPECS = {
        duration: { type: 'number', min: 0,  max: 10,  step: 0.05, placeholder: 'seconds', default: 1 },
        ease:     { type: 'select', options: EASES, default: 'power3.inOut' },
        delay:    { type: 'number', min: 0,  max: 10,  step: 0.05, placeholder: 'seconds', default: 0 },

        x:        { type: 'text',   placeholder: 'e.g. 100 or "50%"' },
        y:        { type: 'text',   placeholder: 'e.g. 100 or "50%"' },
        z:        { type: 'text',   placeholder: 'e.g. 100' },
        xPercent: { type: 'number', min: -200, max: 200, step: 1, placeholder: '-100 to 100' },
        yPercent: { type: 'number', min: -200, max: 200, step: 1, placeholder: '-100 to 100' },
        ...
        skewX:    { type: 'number', min: -90, max: 90, step: 1, placeholder: 'degrees' },
        skewY:    { type: 'number', min: -90, max: 90, step: 1, placeholder: 'degrees' },
    ...
    function presetField(label, withInfo, type, propName, options, value, attrs) {
        ...
        let input;
        if (type === 'select') {
            input = `<select class="pt-input" data-prop="${propName}">${
    ...
    function buildPropInputHtml(spec, initial) {
        if (spec.type === 'select') {
            return `<select class="pt-prop-value" data-type="select">${
                spec.options.map(o => `<option value="${esc(o)}"${String(o) === String(initial) ? ' selected' : ''}>${esc(o)}</option>`).join('')
            }</select>`;
        }
        if (spec.type === 'color') {
        ...
        if (spec.type === 'number') {
            ...
            if (spec.placeholder)        parts.push(`placeholder="${esc(spec.placeholder)}"`);
    ...
    function openPropertyPicker(btn) {
        ...
        pop.innerHTML = `
            ...
            <div class="pt-pop-body">
                ${Object.entries(PROP_CATEGORIES).map(([cat, props]) => `
    ...
    function populateCustomBlock(block, data) {
        ...
        if (list && data.props) {
            Object.entries(data.props).forEach(([key, value]) => addPropRow(list, key, value));
        }
    }

    function saveCustomPane(pane) {
        const enter = collectCustomBlock(pane.querySelector('[data-phase="enter"]'));
    ...
    function renderCustomPane(pane) {
        pane.innerHTML = customBlock('Enter', 'enter') + customBlock('Exit', 'exit');
    ...
    function openExportView(modal) {
        ...
        btnCopy.addEventListener('click', async () => {
            ...
            setTimeout(() => { btnCopy.innerHTML = orig; }, 1200);
        });
    }
    ...
    function buildBuilderUI() {
        ...
        trigger.type = 'button';
        trigger.innerHTML = '<span class="pt-trigger-dot"></span><span>Transition Builder</span>';
        ...
        tabs.forEach(tab => {
            tab.addEventListener('click', () => {
                ...
                panes.forEach(p => p.classList.toggle('active', p.dataset.pane === which));
        ...
        if (getSelected() === 'custom') {
            tabs.forEach(t => t.classList.toggle('active', t.dataset.tab === 'custom'));
            panes.forEach(p => p.classList.toggle('active', p.dataset.pane === 'custom'));
        }
    ...
    /* ============================================================
       Init — SPA navigation primary, reload-fallback for file://
       ============================================================ */
    function init() {
        ...
        // If we got here via a reload-based fallback, replay the enter animation.
        const arriving = SS.get(FLAG_TRANSIT) === '1';
        const arrivalName = SS.get(FLAG_NAME);
        let arrivalCfg = null;
        try { arrivalCfg = JSON.parse(SS.get(FLAG_CONFIG) || 'null'); } catch (e) {}
        SS.remove(FLAG_TRANSIT);
        SS.remove(FLAG_NAME);
        SS.remove(FLAG_CONFIG);

        const selected = getSelected();
        const enterName = (arriving && arrivalName && TRANSITIONS[arrivalName]) ? arrivalName : selected;
        const enterCfg = (arriving && arrivalCfg) ? arrivalCfg : getConfig(enterName);

        const overlay = buildOverlay(enterName, enterCfg);

        if (arriving) {
            overlay.classList.add('is-active');
            TRANSITIONS[enterName].arrive(overlay, enterCfg);
            const global = getGlobalConfig();
            snapBodyOut(global);
            requestAnimationFrame(() => {
                ...
                requestAnimationFrame(() => {
                    Promise.all([
                        ...
                        overlay.classList.remove('is-active');
                        if (enterName !== selected) buildOverlay(selected);
                    });
                });
            });
        } else {
        ...
        try {
            history.replaceState({ pt: true, href: window.location.href }, '', window.location.href);
        } catch (e) {}
